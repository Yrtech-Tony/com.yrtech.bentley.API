...
var dealActualSaleCount = function (money) {
    if (money && money != null) {
        money = String(money);
        var left = money.split('.')[0], right = money.split('.')[1];
        right = right ? (right.length >= 2 ? '.' + right.substr(0, 2) : '.' + right + '0') : '.00';
        var temp = left.split('').reverse().join('').match(/(\d{1,3})/g);
        return (ActualSaleCount(money) < 0 ? "-" : "") + temp.join(',').split('').reverse().join('') + right;
            ...
            editable: {
                ...
                noeditFormatter: function (value, row, index) {
                    var result = { filed: "Income", value: value };
                     if (roleType != "SHOP") {
                        var html = '<a href="javascript:void(0)" data-name="Income" data-pk="undefined" data-value="" class="editable editable-click">' + dealActualSaleCount(result.value) + '</a>';
                        if (result.value == "") {
                            html = '<a href="javascript:void(0)" data-name="Income" data-pk="undefined" data-value="" class="editable editable-click">0</a>';
                        }
                        return html;
                    } else {
                        return dealActualSaleCount(result.value);
                    }
                }
            }
        }],
...
function Add() {
    ...
    var newRow =  {
        MonthSaleId: simple_uuid(index.toString()),
        YearMonth: '',
        ShopId: '',            
        ActualSaleCount: 0,
        ActualSaleAmt: 0
